...
5. Only when the user explicitly asks to install, fetch and inspect the full SKILL.md. Before writing, confirm its frontmatter has a name matching <name> and a non-empty description — a skill missing its description is written but silently never loads. Warn the user if <name> matches an existing built-in skill, because a vault skill silently shadows the built-in. Then write it under Piem/skills/<name>/SKILL.md. Never execute remote code, never copy hidden scripts, and never overwrite an existing vault skill without confirmation.
