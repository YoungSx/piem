...
2. **读取期间没有状态通知。** 原 `openSession` 直到读取、准备 agent 都完成才通知面板，忙碌标记为 false，也没有加载动画。现在冷选择同步发布 opening 状态，旧标题、正文与草稿保持一致，准备成功后一次提交焦点；失败、取消、快速连选和卸载均有回归。热选择保留摘要刷新而不先绘制旧会话的 loading 帧，且不再排队等无关的冷读取；迟到请求不能抢回焦点。
...
本次无损坏、也没有待恢复运行的样例日志，打开均为零写入。Pi 既有的断尾、缺尾换行修复和中断运行收尾行为没有改变，这些恢复场景仍可能写入。
...
最终 bundle：1,857,820 字节，SHA-256 `4254cbc95ba9b5d86679183db4bf796d487ab95cb119e8e733a05dbc242feb3c`。安装副本与本地构建一致。相对发布基线增加 4,297 字节，无新依赖；按既有规则将 bundle 上限从 1.77 MiB 提至 1.78 MiB，其余依赖组成检查保持通过。
...
最终小样本 service 冷开记录为 9.4 ms，重建后 11.3 ms，热开为 1.5–1.6 ms。这些不含挂载整篇聊天的排版，不能作为用户实际开窗耗时。人为延迟仅用于观察 loading UI，不计入性能结果。
...
本地 `npm run verify` 全部通过：构建、bundle/skills/copy/css/version 门禁、**3,415 项测试（218 个文件）**、lint。新增 sessionOpen、sessionOpening、transcriptUpdates 文件也分别独立运行通过；流式、后台会话、草稿、失败恢复、存储并发均有回归。
...
## 2026-09-11 主分支更新后的复验

PR #406 已 rebase 到主分支 `539c48c0faace983ab5f0c59d2e5da63506243d7`。唯一文本冲突是 `scripts/check-bundle.mjs` 的预算；保留主分支的 **1.80 MiB** 上限、通用组件支持与 `scripts/fixtures/` 禁入规则。会话和消息渲染实现与 rebase 前逐字一致。

合并后生产 bundle 为 **1,884,024 字节**，SHA-256 `400e81f794682150f85e0e942f8edde9d188704afad1739ac6fee450c5188c0f`，未超过已有上限。`bun install --frozen-lockfile` 与完整 `npm run verify` 通过，含 **3,480 项测试、226 个文件、11,626 个断言**及 lint。sessionOpen、sessionOpening、transcriptUpdates、useExtensionUI、nativeComponentHost、native-extension-fixture 六个文件独立运行也全部通过。

合并后的生产产物已安装到新的独立 Vault，在 Xvfb 中重跑实际 Obsidian 1.13.7：**32 项 session smoke 通过**，加载文字与动画推进、旧正文保留、冷读 2 次／热读 0 次、笔记上下文更新、临时服务释放后 runtime 为 0 均已验证；截图已查看。没有用此烟测宣称新增通用组件的专用测试工厂已在生产包中运行。证据在 `/home/ubuntu/piem-session-rebase-20260911/`。烟测已完成后，宿主在外层时限附近记录 `GPU process isn't usable` 并退出；没有据此宣称图形运行问题已解决。父进程确认所属进程均已回收，CDP 端口关闭。
