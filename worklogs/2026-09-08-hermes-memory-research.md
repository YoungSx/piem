...
对 Piem 可借鉴的是：把事实、会话记录、可复用流程分开；让明确的记忆请求和更正立即生效；用小而可靠的写入模块替代反复问批准。Hermes 的自动注入、宿主文件系统、后台模型副本、固定字符数字都不能直接等同于 Piem 的设计选择。
...
4. **工程护栏放在数据完整性。** 小 memory 模块提供读、增、改、移除/恢复；Vault 内路径约束、局部串行、写前读最新、截断不覆盖、最终容量验收、明确错误结果。不要把“是否值得记”做成分数计算器或观察次数门禁。
5. **先兑现已有读取路线。** 如果 Piem 继续保持已确定的按需 `read`，就把候选和长期索引做得容易读、容易更新；不要因为 Hermes 自动注入就擅自推翻这一产品决定。自动注入需要独立衡量上下文成本、偏好新鲜度与来源权威。
