...
- **Release with `npm run release -- patch` (or `minor` / `major` / an explicit version).** It is the whole flow: bump, gates, commit, tag, push. Add `--dry-run` to stop before the commit and see what it would do.
