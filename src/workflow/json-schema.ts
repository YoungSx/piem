 ...
 *
 * Ported from tintinweb/pi-subagents' `json-schema.ts`. `agent(prompt, { schema })`
 * hands us a raw JSON Schema written by a model, used as the check that decides
 * whether what came back is usable.
 *
 * **`typebox`, not `@sinclair/typebox`.** They are different packages. The
 * `@sinclair` package (0.34) dispatches on a `Kind` symbol a schema arriving
 * over the wire does not carry, so `Value.Check` throws `Unknown type` on a
 * plain JSON Schema. `typebox` v1 is a standards JSON Schema validator and
 * takes the schema as-is — and it is the package pi itself types tool
 * parameters against, so the same schema object serves both roles.
 *
...
export interface CompiledSchema {
	/** The schema as given, for the journal key and the answer instruction. */
 ...
 * not an object cannot be an answer contract at all, so it would break every
...
export function compileJsonSchema(schema: unknown): SchemaCompilation {
	...
	if (root.type !== "object") {
		return {
			ok: false,
			message:
				'agent() opts.schema must have `type: "object"` at its root — the child is asked to answer with an object, '
