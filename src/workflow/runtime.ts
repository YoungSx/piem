...
import { type JournalKeyInput, journalKey, type WorkflowJournalEntry } from "./journal";
import { type CompiledSchema, compileJsonSchema } from "./json-schema";
import { extractMeta, type WorkflowMeta } from "./meta";
...
/**
 * Concurrent agents allowed, leaving headroom for the host and the UI.
 *
 * `Math.max(1, …)` is not decoration: the raw `min(16, cores - 2)` is 0 on a one-
 * or two-core device, and a semaphore with zero permits never hands out a slot,
 * so the run would hang before its first agent rather than fail. The core count
 * comes from `navigator.hardwareConcurrency` (upstream read node's `cpus()`),
 * which a WebView reports; a phone that hides it falls back to 4.
 */
export function workflowConcurrency(cpuCount: number = navigatorConcurrency()): number {
  return Math.max(1, Math.min(16, cpuCount - 2));
}

/** Cores the WebView admits to, or 4 when it hides the number (Safari caps/omits it). */
...
function walk(value: unknown, path: string, seen: Set<object>): void {
  ...
  if (kind !== "object") throw boundaryError(`a ${String(kind)}`, path);
  ...
  if (Array.isArray(object)) {
    for (let i = 0; i < object.length; i++) {
      if (!Object.prototype.hasOwnProperty.call(object, i)) throw boundaryError("a sparse array", `${path}[${i}]`);
...
/**
 * True when `text` holds a control character — everything below 0x20 except
 * tab (0x09), newline (0x0A) and carriage return (0x0D), plus DEL (0x7F).
 *
 * A char-code scan rather than a control-character regex literal: the Obsidian
 * plugin guidelines (and this repo's lint) reject `no-control-regex`, and the
 * scan says exactly what it allows without a literal full of escapes.
 */
...
export async function runWorkflow(options: RunWorkflowOptions): Promise<WorkflowRunResult> {
  ...
  // Called through this closure everywhere below rather than referenced bare:
  // a bare method reference off `options.journal` trips the unbound-method lint.
  const recordJournal = options.journal?.append
    ? (entry: WorkflowJournalEntry): void => { options.journal?.append?.(entry); }
    : undefined;
  /**
  ...
  // A blob-URL Web Worker, not node's `new Worker(src, {eval})`: the WebView
  // has no worker_threads. `workerData` has no equivalent either, so the init
  // payload is posted as the first message — the worker buffers it until its
  // synchronously-registered handler drains it. `meta` is not sent: it is
  // host-side only (the progress groups seed from it here), and the worker
  // never reads it.
  ...
  return await new Promise<WorkflowRunResult>(resolve => {
    ...
    const finish = (result: Omit<WorkflowRunResult, "meta" | "progress" | "agentCount" | "replayedCount">) => {
      ...
      inflight.clear();
      semaphore.drain();
      // A Web Worker's terminate() is synchronous and returns void (node's
      // returned a Promise). Revoke the blob URL on the way out so a long
      // session running many workflows does not leak object URLs.
      worker.terminate();
    ...
    async function handleAgent(callId: number, payload: AgentCallPayload): Promise<void> {
      ...
      // journalKey is async here (Web Crypto SHA-256 has no sync form), so the
      // key is awaited once up front and the same value reused below — computing
      // it twice would hash twice for nothing.
      const key = await journalKey(keyInput);
      ...
      // `key` was computed once above (awaited); reused here for the journal.
    ...
    // A script that throws at parse or an uncaught worker error surfaces here;
    // a Web Worker has no separate "exit" event, so onerror is the only crash
    // signal and it must settle the run rather than leave it hanging.
    worker.onerror = (event: ErrorEvent) => {
      finish({ status: "failed", error: event.message || "Workflow worker errored before completing." });
    };
  });
}
