...
export const zhCN: DeepPartial<EnCopy> = {
	...
	extensions: {
		...
		catalog: {
			webSearchName: "联网搜索",
			webSearchDesc: "让助手通过当前服务商联网搜索。查询和指定网址会发给该服务商，可能额外计费。",
			modelSwitchName: "模型切换",
			modelSwitchDesc: "让助手在对话中途切换到另一个已配置的模型。",
			contextName: "上下文管理",
			contextDesc: "给助手自助检查点、压缩、回顾上下文的工具。/context 视图不受影响。",
			agentTeamName: "智能体团队",
			agentTeamDesc: "增加团队工具，运行带独立信箱与审计记录的多个 Pi 智能体。",
			clarifyName: "草稿改写",
			clarifyDesc: "发送前把口语草稿改写成精确 prompt（用 /clarify 或 -clarify 标记）。会把草稿发给选定模型。",
			continueName: "无声继续",
	...
	chat: {
		...
		busySending: "Piem 正在接收问题，请稍等再发送。",
	...
	noteReference: {
		cards: "附带的引用",
		kind: { file: "文件", folder: "文件夹", url: "网页", selection: "选区" },
		remove: "移除引用：{name}",
		lines: "第 {start}–{end} 行",
		pathHint: "发送问题后，Piem 可读取这个文件或文件夹。",
		webHint: "发送问题后，Piem 可请求这个网页。",
		referenceLimit: "最多附带 {limit} 个引用，引用资料合计不超过 20,000 个字符。请移除部分引用后重试。",
		missingCount: "所选项目中有 {count} 项已不可用。",
		extensionReferences: "这些引用用于提问。请发送问题，或先移除引用再运行此命令。",
	...
	skills: {
		...
		userHeading: "用户级技能",
		userDesc: "技能也可以从这台电脑上、笔记库之外的文件夹加载。下方的已搜索列表会列出实际读取了哪些文件夹。",
	...
	signIn: {
		title: "登录到{target}",
		method: "此提供方通过 {method} 接入。",
		signedIn: "已登录，此提供方可以使用了。",
		signedOut: "尚未登录。",
		unavailable: "此设备无法把登录凭据存到库外，因此这里用不了订阅登录。请改用 API 密钥添加此提供方。",
		start: "登录",
		again: "重新登录",
		signOut: "退出登录",
		close: "关闭",
		...
		codeCopied: "登录码已复制。",
		openPage: "请打开此页面继续：",
		openPageButton: "打开登录页面",
		waiting: "正在等待你确认授权，此窗口可以一直开着。",
		done: "已登录。",
		doneNotice: "已登录到{target}。",
		failed: "登录失败。{message}",
		signedOutNotice: "已退出{target}的登录。",
		signOutFailed: "无法退出登录。{message}",
		promptUnsupported: "此登录流程需要当前版本无法展示的输入（{kind}）。",
		rowAction: "登录或退出",
		rowSignedIn: "已登录",
		rowSignedOut: "未登录",
		modalNote: "此提供方使用登录而不是 API 密钥。请先保存，再用它那一行的登录操作。",
	},
	...
	subagents: {
		...
		scopeAria: "显示哪些对话",
		groupThisChat: "本对话",
		groupUnknownChat: "已关闭的对话",
		stopChat: "停止本对话的",
		stopChatAria: "停止 {chat} 中所有正在运行的子代理",
		stopEverything: "全部停止 {count} 个（{chats} 个对话）",
		stopEverythingAria: "停止所有对话中正在运行的全部子代理",
		emptyHereHint: "本对话没有派出过子代理。有 {count} 个属于其他对话——切到「所有对话」就能看到。",
		fromChat: "由{chat}派出",
		status: {
			running: "进行中",
			done: "已完成",
