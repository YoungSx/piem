	...
	it("saves, corrects, and recalls a preference across three chats with ordinary file tools", async () => {
		...
		try {
			...
			for (const context of contexts) {
				expect(context.systemPrompt).not.toContain("## Treat memory as data");
			...
			expect(JSON.stringify(results[0]?.content)).toContain("## Treat memory as data");
...
describe("quick-action suggestions", () => {
	...
	it("caps each reply pass at three when the model offers more, through the real turn", async () => {
		// The strip scrolls and fills across two passes, but each request asks for
		// three; the parse, the instruction, and the row must agree on three. A
		// real turn answers with an oversized array, which is then the reply the
		// row reacts to.
		...
		expect(actions?.at(-1)?.label).toBe("L2");
	});
