...
在文件的菜单中选择 **询问此文件**，就能将它固定在当前对话的上下文中。文件夹
提供 **询问此文件夹**，将目录路径加入草稿；发送问题后，Piem 可列出和搜索这个
目录。在文件列表里多选文件或文件夹，则会出现 **询问所选项目**。

每段对话最多固定八个文件。超出的文件会和选中的文件夹一起列入草稿，并提示
哪些是新固定、已经固定、加入草稿或不可用的项目。原有草稿会保留。如果合并后的
草稿将超过 20,000 字符的存储上限，本次追加会被拒绝并说明原因；先发送或缩短
草稿，再加入这些引用。

保存的草稿加载期间，输入框暂时只读。手机上加入引用会展开折叠的输入框，并滚动
到草稿末尾，方便接着输入问题。

编辑器里，选中文字时提供 **询问所选内容**，没有选区时提供 **询问此笔记**。
外部 HTTP 或 HTTPS 链接提供 **询问此链接**，将网址加入草稿。这些操作只准备
提问材料，你仍需输入问题并选择 **发送**。打开链接菜单不会抓取网页；发送后，
`web_fetch` 可请求该网页，受网站访问权限和工具文本响应上限约束。

当前笔记有打开的编辑器时，Piem 读取其中的最新文字，包括未保存的修改；已关闭
的笔记读取库中的保存版本。菜单跟随 Obsidian 提供的入口，不保证每个自定义视图
或手机手势都提供全部项目。
